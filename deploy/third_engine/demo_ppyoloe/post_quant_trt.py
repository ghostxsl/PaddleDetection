...
def check_network(network):
    ...
    max_len = max([len(inp.name)
                   for inp in inputs] + [len(out.name) for out in outputs])
...
def create_optimization_profiles(builder,
                                 inputs,
    ...
    for bs in batch_sizes:
        ...
        for inp in inputs:
            ...
            profiles[bs].set_shape(
                inp.name, min=(bs, *shape), opt=(bs, *shape), max=(bs, *shape))

    return list(profiles.values())


def parse_input_shape(input_shape):
    assert isinstance(input_shape, str)
    return [int(a) for a in input_shape.split(',')]
...
def main():
    ...
    parser.add_argument(
        "--gpu_fallback",
        action='store_true',
        help="Set trt.BuilderFlag.GPU_FALLBACK.")
    parser.add_argument(
        "--refittable", action='store_true', help="Set trt.BuilderFlag.REFIT.")
    parser.add_argument(
        "--debug", action='store_true', help="Set trt.BuilderFlag.DEBUG.")
    parser.add_argument(
        "--strict_types",
        action='store_true',
        help="Set trt.BuilderFlag.STRICT_TYPES.")
    parser.add_argument(
        "--fp16",
        action="store_true",
    ...
    parser.add_argument(
        "--int8",
        action="store_true",
        help="Attempt to use INT8 kernels when possible. "
        "This should generally be used in addition to the --fp16 flag. "
    ...
    parser.add_argument(
        "--just_image",
        action="store_true",
        help="Input: only one single input `image`")
    parser.add_argument(
        "--input_shape",
        type=str,
        default="3,640,640",
        help="the shape of input to the model")
    ...
    with trt.Builder(TRT_LOGGER) as builder, \
        ...
        if args.int8:
            ...
            config.int8_calibrator = get_int8_calibrator(
                ...
                args.max_calibration_size, args.calibration_batch_size,
                parse_input_shape(args.input_shape), args.just_image)
        ...
        # with builder.build_engine(network, config) as engine, open(args.output,
        #                                                            "wb") as f:
        #     logger.info("Serializing engine to file: {:}".format(args.output))
