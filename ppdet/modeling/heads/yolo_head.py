...
class YOLOv3Head(nn.Layer):
    ...
    def __init__(self,
        ...
        if self.use_decoupled_head:
            ...
            for i, in_c in enumerate(self.in_channels):
                out_c = len(self.anchors[i]) * self.num_classes
                self.pred_cls.append(
                    nn.Sequential(*[
                        ConvBNLayer(
                            in_c, 256, padding=1, act='mish'), ConvBNLayer(
                                256, 256, padding=1, act='mish'), nn.Conv2D(
                                    256, out_c, 3, padding=1)
                    ]))
                out_c = len(self.anchors[i]) * 6 if self.iou_aware else len(
                    self.anchors[i]) * 5
    ...
    def forward(self, feats, targets=None):
        ...
        for i, feat in enumerate(feats):
            if self.use_decoupled_head:
                b, c, h, w = get_static_shape(feat)
                cls_logit = self.pred_cls[i](feat)
                reg_pred = self.pred_reg[i](feat)
                cls_logit = cls_logit.reshape(
                    [b, len(self.anchors[i]), self.num_classes, h, w])
                c = 6 if self.iou_aware else 5
                reg_pred = reg_pred.reshape([b, len(self.anchors[i]), c, h, w])
                yolo_output = paddle.concat([reg_pred, cls_logit], axis=2)
                yolo_output = yolo_output.reshape(
                    [b, len(self.anchors[i]) * (self.num_classes + c), h, w])
            else:
...
class PPTSimOTAHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.tood_cls = nn.Conv2D(feat_channels, self.num_classes, 3, padding=1)
        self.tood_reg = nn.Conv2D(feat_channels, 4, 3, padding=1)

        self.cls_prob_conv1 = nn.Conv2D(feat_channels * self.stacked_convs,
                                        feat_channels // 4, 1)
        self.cls_prob_conv2 = nn.Conv2D(feat_channels // 4, 1, 3, padding=1)
        self.reg_offset_conv1 = nn.Conv2D(feat_channels * self.stacked_convs,
                                          feat_channels // 4, 1)
        self.reg_offset_conv2 = nn.Conv2D(
            feat_channels // 4, 4 * 2, 3, padding=1)
    ...
    def forward(self, feats, targets=None):
        ...
        anchors, anchor_points, num_anchors_list, stride_tensor_list = \
            generate_anchors_for_grid_cell(feats, self.fpn_strides, self.grid_cell_scale,
            self.grid_cell_offset)
        ...
        for feat, conv_reduction, scale_reg, anchor, stride in zip(
                feats, self.stem_conv, self.scales_regs, anchors_split,
                self.fpn_strides):
...
class PPDeHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.stem_conv = nn.LayerList()
        for in_c in self.in_channels:
            self.stem_conv.append(ConvBNLayer(in_c, feat_channels, 1, act=act))
        # pred head
        ...
        for _ in self.in_channels:
            self.conv_cls.append(
                nn.Sequential(*[
                    ConvBNLayer(
                        feat_channels, feat_channels, padding=1,
                        act=act), ConvBNLayer(
                            feat_channels, feat_channels, padding=1, act=act),
                    nn.Conv2D(
                        feat_channels, self.num_classes, 3, padding=1)
                ]))
    ...
    def _init_weights(self):
        ...
        for cls_, reg_ in zip(self.conv_cls, self.conv_reg):
            ...
            constant_(reg_[-1].bias, bias_reg)
