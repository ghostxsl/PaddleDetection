...
class PPRefineHead(nn.Layer):
    ...
    def _init_weights(self):
        ...
        anchor_centers, stride_tensor = self._generate_anchors()
        self.register_buffer('stride_tensor', stride_tensor)
        for i, anchor_center in enumerate(anchor_centers):
            self.register_buffer(f'anchor_center{i}', anchor_center)
    ...
    def _generate_anchors(self):
        ...
        for i, stride in enumerate(self.fpn_strides):
            ...
            anchor_point = paddle.cast(
                paddle.stack([shift_x, shift_y]), dtype='float32')
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            # reg prediction and alignment
            anchor_center = getattr(self, f'anchor_center{i}')
            reg_bbox = self._distance2bbox(anchor_center, reg_dist)
    ...
    def post_process(self, head_outs, img_shape, scale_factor):
        ...
        pred_bboxes = pred_bboxes.transpose([0, 2, 1]) * self.stride_tensor
        # clip bbox to origin
...
class PPRefineTHead(nn.Layer):
    ...
    def __init__(self,
        ...
        for in_c in self.in_channels:
            ...
            self.simT_reg.append(nn.Conv2D(in_c, 4, 3, padding=1))
        # refine head
        self.conv_refine = nn.LayerList()
        for in_c in self.in_channels:
            self.conv_refine.append(nn.Conv2D(in_c, 5, 1))
    ...
    def _init_weights(self):
        ...
        bias_init = paddle.zeros([5])
        bias_init[0] = bias_cls
        for conv_ in self.conv_refine:
            constant_(conv_.weight)
            conv_.bias.set_value(bias_init)
    ...
    def forward_train(self, feats, targets):
        ...
        for i, feat in enumerate(feats):
            ...
            cls_logit = self.simT_cls[i](self.stem_cls[i](feat, avg_feat))
            reg_dist = self.simT_reg[i](self.stem_reg[i](feat, avg_feat))
            cls_refine, reg_refine = self.conv_refine[i](feat).split(
                [1, 4], axis=1)
            # cls refine
            cls_score = (F.sigmoid(cls_logit) * F.sigmoid(cls_refine)).sqrt()
            cls_score_list.append(cls_score.flatten(2).transpose([0, 2, 1]))
            # reg refine
            reg_refine = reg_refine.exp() * F.relu(reg_dist)
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        pred_bboxes = batch_distance2bbox(anchor_points_s, pred_dist)
        refine_bboxes = batch_distance2bbox(anchor_points_s, pred_refine)
        gt_labels = gt_meta['gt_class']
        gt_bboxes = gt_meta['gt_bbox']
        pad_gt_mask = gt_meta['pad_gt_mask']
        gt_scores = gt_meta['gt_score'] if 'gt_score' in gt_meta else None
        ...
        loss_l1_r, loss_iou_r = self._bbox_loss(
            refine_bboxes, assigned_labels, assigned_bboxes, assigned_scores,
            assigned_scores_sum)
        loss = self.loss_weight['class'] * loss_cls + \
               self.loss_weight['iou'] * loss_iou + \
...
class PPRefineDHead(nn.Layer):
    ...
    def __init__(self,
        ...
        super(PPRefineDHead, self).__init__()
        ...
        # refine head
        self.refine_head = nn.LayerList()
        for in_c in self.in_channels:
            self.refine_head.append(
                nn.Conv2D(in_c, 1 + 4 * (self.reg_max + 1), 1))
    ...
    def _init_weights(self):
        ...
        bias_init = paddle.zeros([1 + 4 * (self.reg_max + 1)])
        bias_init[0] = bias_cls
        for refine_ in self.refine_head:
            constant_(refine_.weight)
            refine_.bias.set_value(bias_init)
    ...
    def forward_train(self, feats, targets):
        ...
        for i, feat in enumerate(feats):
            ...
            cls_refine, reg_refine = self.refine_head[i](feat).split(
                [1, 4 * (self.reg_max + 1)], axis=1)
            # cls refine
            cls_score = (F.sigmoid(cls_logit) * F.sigmoid(cls_refine)).sqrt()
            cls_score_list.append(cls_score.flatten(2).transpose([0, 2, 1]))
            # reg refine
            reg_dist_list.append(reg_dist.flatten(2).transpose([0, 2, 1]))
            reg_refine = reg_refine.exp() * reg_dist
            refine_dist_list.append(reg_refine.flatten(2).transpose([0, 2, 1]))

        cls_score_list = paddle.concat(cls_score_list, axis=1)
        reg_dist_list = paddle.concat(reg_dist_list, axis=1)
        refine_dist_list = paddle.concat(refine_dist_list, axis=1)

        return self.get_loss([
            cls_score_list, reg_dist_list, refine_dist_list, anchors,
            anchor_points, num_anchors_list, stride_tensor
        ], targets)
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            # reg refine
            reg_refine = reg_refine.exp() * reg_dist
            reg_dist_list.append(reg_refine.reshape([b, -1, l]))
    ...
    def _bbox2distance(self, points, bbox, max_dist):
        x1y1, x2y2 = paddle.split(bbox, 2, -1)
        lt = points - x1y1
        rb = x2y2 - points
        return paddle.concat([lt, rb], -1).clip(0, max_dist - 0.01)
    ...
    def _bbox_loss(self, pred_dist, pred_bboxes, anchor_points, assigned_labels,
        ...
        if num_pos > 0:
            ...
            loss_dfl = loss_dfl.mean(-1).sum() / assigned_scores_sum
        else:
            loss_l1 = paddle.zeros([1])
            loss_iou = paddle.zeros([1])
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        anchor_points_s = anchor_points / stride_tensor
        pred_bboxes = self._bbox_decode(anchor_points_s, pred_dist)
        refine_bboxes = self._bbox_decode(anchor_points_s, pred_refine)
        ...
        loss_l1, loss_iou, loss_dfl = \
            self._bbox_loss(pred_dist, pred_bboxes, anchor_points_s,
                            assigned_labels, assigned_bboxes, assigned_scores,
                            assigned_scores_sum)
        loss_l1_r, loss_iou_r, loss_dfl_r = \
            self._bbox_loss(pred_refine, refine_bboxes, anchor_points_s,
                            assigned_labels, assigned_bboxes, assigned_scores,
                            assigned_scores_sum)
        loss = self.loss_weight['class'] * loss_cls + \
               self.loss_weight['iou'] * loss_iou + \
               ...
               self.loss_weight['iou_r'] * loss_iou_r + \
               self.loss_weight['dfl_r'] * loss_dfl_r
        out_dict = {
            'loss': loss,
            'loss_cls': loss_cls,
            'loss_iou': loss_iou,
