...
class PPYOLOTHead(nn.Layer):
    ...
    def __init__(self,
        ...
        for in_c in in_channels:
            self.stem_cls.append(
                nn.Sequential(
                    ConvNormLayer(
                        in_c, in_c, data_format=data_format),
                    ConvNormLayer(
                        in_c, in_c, data_format=data_format)))
        ...
        for in_c in in_channels:
            self.pred_cls.append(
                nn.Conv2D(
                    in_c, self.num_classes, 1, data_format=data_format))
            self.pred_reg.append(
                nn.Conv2D(
                    in_c, 4 * (self.reg_max + 1), 1, data_format=data_format))
    ...
    def forward_train(self, feats, targets):
        ...
        cls_score_list, reg_distri_list = [], []
        for i, feat in enumerate(feats):
            cls_logit = self.pred_cls[i](self.stem_cls[i](feat) + feat)
