...
class ConvNormLayer(nn.Layer):
    ...
    def forward(self, x):
        x = self.conv(x)
        if self.data_format == "NCHW" and 'LayerNorm' in self.norm_layer:
...
class PPYOLOTHead(nn.Layer):
    ...
    def __init__(self,
        ...
        assert data_format in ["NCHW", "NHWC"], \
            f"{self.data_format} format is not supported."
        ...
        self.stem_cls = nn.Sequential(
            ConvNormLayer(
                in_channels, in_channels, data_format=data_format),
            ConvNormLayer(
                in_channels, in_channels, data_format=data_format))
        ...
        self.pred_cls = nn.Conv2D(
            in_channels, self.num_classes, 1, data_format=data_format)
        self.pred_reg = nn.Conv2D(
            in_channels, 4 * (self.reg_max + 1), 1, data_format=data_format)

        # projection conv
        self.proj_conv = nn.Conv2D(
            self.reg_max + 1, 1, 1, bias_attr=False, data_format=data_format)
        self.proj_conv.skip_quant = True
        self._init_weights()
    ...
    def _init_weights(self):
        ...
        constant_(self.pred_reg.bias, 1.0)
    ...
    def forward_train(self, feats, targets):
        anchors, anchor_points, num_anchors_list, stride_tensor = \
            generate_anchors_for_grid_cell(
                feats, self.fpn_strides, self.grid_cell_scale,
                self.grid_cell_offset, data_format=self.data_format)

        cls_score_list, reg_distri_list = [], []
        for i, feat in enumerate(feats):
            cls_logit = self.pred_cls(self.stem_cls(feat) + feat)
            reg_distri = self.pred_reg(self.stem_reg(feat))
            # cls and reg
            cls_score = F.sigmoid(cls_logit)
            if self.data_format == "NCHW":
                cls_score_list.append(cls_score.flatten(2).transpose([0, 2, 1]))
                reg_distri_list.append(
                    reg_distri.flatten(2).transpose([0, 2, 1]))
            else:
                cls_score_list.append(cls_score.flatten(1, 2))
                reg_distri_list.append(reg_distri.flatten(1, 2))
        cls_score_list = paddle.concat(cls_score_list, axis=1)
        reg_distri_list = paddle.concat(reg_distri_list, axis=1)
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            l = h * w
            cls_logit = self.pred_cls(self.stem_cls(feat) + feat)
            reg_dist = self.pred_reg(self.stem_reg(feat))
            if self.data_format == "NCHW":
                ...
                reg_dist = reg_dist.reshape([-1, l, 4, self.reg_max + 1])
                reg_dist = self.proj_conv(F.softmax(reg_dist)).squeeze(-1)
            # cls and reg
            cls_score = F.sigmoid(cls_logit)
            if self.data_format == "NCHW":
                cls_score = cls_score.reshape([b, self.num_classes, l])
            else:
    ...
    def _bbox_decode(self, anchor_points, pred_dist):
        ...
        pred_dist = F.softmax(pred_dist.reshape([b, l, 4, self.reg_max + 1]))
        if self.data_format == "NCHW":
            pred_dist = self.proj_conv(pred_dist.transpose([0, 3, 1, 2
                                                            ])).squeeze(1)
        else:
