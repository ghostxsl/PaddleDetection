...
class TOODHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cls_prob_conv1 = nn.Conv2D(self.feat_channels * self.stacked_convs,
    ...
    def _normalize_grid_coordinates(self, reference_points, offset, shape):
        b, _, h, w = shape
        normalize_shape = paddle.to_tensor([w, h]).reshape([1, 1, 1, 2])
        reference_points = reference_points.reshape([1, h, w, 2])
        offset = offset.reshape([b * 4, 2, h, w]).transpose([0, 2, 3, 1])
        reg_coord = reference_points + offset
        reg_coord = (reg_coord / normalize_shape) * 2 - 1
    ...
    def forward(self, feats):
        ...
        for feat, scale_reg, anchor, stride in zip(feats, self.scales_regs,
            ...
            reg_bbox = reg_bbox.reshape([b, h, w, 4]).transpose([0, 3, 1, 2])
            ...
            if not self.training:
                bbox_pred *= stride
            bbox_pred_list.append(bbox_pred.flatten(2).transpose([0, 2, 1]))
    ...
    def _task_aligned_focal_loss(self,
                                 pred_scores,
                                 target_scores,
                                 target_labels,
                                 gamma=2.0):
        mask_positive = (target_labels != self.num_classes).unsqueeze(-1).tile(
            [1, 1, self.num_classes])
        mask_negative = (target_labels == self.num_classes).unsqueeze(-1).tile(
            [1, 1, self.num_classes])
        weight = (pred_scores - target_scores).pow(gamma)
        loss_pos = F.binary_cross_entropy(
            paddle.masked_select(pred_scores, mask_positive),
            paddle.masked_select(target_scores, mask_positive),
            weight=paddle.masked_select(weight, mask_positive),
            reduction='sum')
        loss_neg = F.binary_cross_entropy(
            paddle.masked_select(pred_scores, mask_negative),
            paddle.masked_select(target_scores, mask_negative),
            weight=paddle.masked_select(weight, mask_negative),
            reduction='sum')
        return loss_pos + loss_neg

    @staticmethod
    def focal_loss(score, label, alpha=0.25, gamma=2.0):
        bce_loss = F.binary_cross_entropy(score, label, reduction="none")
        p_t = score * label + (1 - score) * (1 - label)
        loss = bce_loss * ((1 - p_t).pow(gamma))
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        if gt_meta['epoch_id'] < self.static_assigner_epoch:
            ...
            # classification loss
            loss_cls = self.focal_loss(pred_scores, assigned_scores)
        else:
            assigned_labels, assigned_bboxes, assigned_scores = self.assigner(
                ...
                bg_index=self.num_classes)
            # classification loss
            loss_cls = self._task_aligned_focal_loss(
                pred_scores, assigned_scores, assigned_labels)
