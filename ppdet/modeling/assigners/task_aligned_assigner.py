...
class TaskAlignedAssigner(nn.Layer):
    ...
    def __init__(self,
        ...
        self.use_bipartite_match = use_bipartite_match
        self.eps = eps

    def bipartite_match(self, cost_matrix):
        out_match = []
        for i in range(len(cost_matrix)):
            m = cost_matrix[i].numpy()
            row, col = linear_sum_assignment(m, True)
            t = np.zeros_like(m)
            t[row, col] = 1
            out_match.append(t)
        out_match = np.stack(out_match)
        return paddle.to_tensor(out_match)
    ...
    def forward(self,
        ...
        if self.use_bipartite_match:
            bipartite_match = self.bipartite_match(
                alignment_metrics) * pad_gt_mask
        ...
        if mask_positive_sum.max() > 1:
            ...
            mask_positive_sum = mask_positive.sum(axis=-2)
        if self.use_bipartite_match:
            mask_bipartite = bipartite_match.sum(axis=-2, keepdim=True)
            mask_positive = paddle.where(mask_bipartite == 1, bipartite_match,
                                         mask_positive)
