...
from .utils import (compute_max_iou_gt, check_points_inside_bboxes,
...
class SimOTAv2Assigner(nn.Layer):
    ...
    def _get_dynamic_topk(self, metrics, ious):
        batch_size, num_max_boxes, num_anchors = metrics.shape
        topk_ious, _ = paddle.topk(ious, self.topk + 1, axis=-1)
        dynamic_topk = topk_ious.sum(-1).astype('int32').clip(max=self.topk)
        ...
        batch_ind = paddle.stack(
            ...
            axis=-1)
        topk_metrics, _ = paddle.topk(metrics, self.topk + 1, axis=-1)
        dynamic_threshold = paddle.gather_nd(topk_metrics,
                                             batch_ind).unsqueeze(-1)
        is_in_topk = paddle.where(metrics > dynamic_threshold,
                                  paddle.ones_like(metrics),
                                  paddle.zeros_like(metrics))
    ...
    def forward(self,
        ...
        3. limit the positive sample's center in gt or center_radius
            (because the anchor-free detector only can predict positive distance)
        ...
        if num_max_boxes == 0:
            ...
            assigned_ious = paddle.zeros([batch_size, num_anchors, num_classes])
        ...
        is_in_intersection, _ = check_points_inside_bboxes(
            anchor_points, gt_bboxes, stride_tensor)

        # select topk largest alignment metrics pred bbox as candidates
        # for each gt, [B, n, L]
        is_in_topk = self._get_dynamic_topk(loss_metrics * is_in_intersection,
                                            ious * is_in_intersection)
        ...
        if mask_positive_sum.max() > 1:
            mask_multiple_gts = (mask_positive_sum.unsqueeze(1) > 1).tile(
                [1, num_max_boxes, 1])
            is_max_iou = compute_max_iou_anchor(loss_metrics)
        ...
        assigned_ious = F.one_hot(assigned_labels, num_classes)
        ious *= mask_positive
