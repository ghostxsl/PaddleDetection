...
from ppdet.modeling.bbox_utils import nonempty_bbox, rbox2poly, compute_bbox3d_corners
...
class FCOSMono3DPostProcess(object):
    ...
    def _decode_fcos_mono3d(self, cls_logits, bboxes_reg, centerness,
        ...
        inv_camera_instrinsic = paddle.stack(inv_camera_instrinsic).astype(
            paddle.float32)
        inv_camera_instrinsic[:, 0, :] /= scale_factor[:, :, 0]
        inv_camera_instrinsic[:, 1, :] /= scale_factor[:, :, 1]
        K_inv = np.linalg.pinv(inv_camera_instrinsic.numpy())
        K_inv = paddle.to_tensor(K_inv).unsqueeze(1)

        bboxes = compute_bbox3d_corners(center, depth, size, rotation_yaw,
                                        K_inv)

        # bird view bbox
        bboxes = bboxes.transpose([0, 1, 3, 2])[:, :, :4, :]
        bboxes = paddle.stack([bboxes[:, :, :, 0], bboxes[:, :, :, 2]], axis=-1)
        bboxes = bboxes.reshape([bboxes_reg.shape[0], bboxes_reg.shape[1], -1])

        return bboxes, score, bboxes_reg

    def __call__(self, fcos_head_outs, scale_factor, inv_camera_instrinsic):
        cls_logits, bboxes_reg, centerness, direction_logits, anchor_points = fcos_head_outs
        bboxes, score, bboxes_reg_pred = self._decode_fcos_mono3d(
            cls_logits, bboxes_reg, centerness, direction_logits, anchor_points,
            scale_factor, inv_camera_instrinsic)
        bbox_pred, bbox_num, bbox_index = self.nms(bboxes, score)
