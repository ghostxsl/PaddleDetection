  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    for (int l_col=0; l_col < num_levels; ++l_col)
    {
      ...
      for (int p_col=0; p_col < num_points; ++p_col)
      {
        ...
        {
          col += deformable_attn_bilinear_forward(data_value_ptr,
...
// forward
std::vector<paddle::Tensor> MSDeformableAttnCUDAForward(
    ...
    output.reshape({batch_size, query_length, num_heads, channels});
    ...
    <<<GET_BLOCKS(num_kernels, CUDA_NUM_THREADS), CUDA_NUM_THREADS,
        ...
        query_length, num_points, output.mutable_data<float>());
...
__global__ void deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1(
    const int n,
    const data_t *grad_col,
    const data_t *data_value,
    const int64_t *data_spatial_shapes,
...
template <typename data_t, unsigned int blockSize>
__global__ void deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2(
    const int n,
    const data_t *grad_col,
    const data_t *data_value,
    const int64_t *data_spatial_shapes,
    ...
    data_t* grad_attn_weight)
{
  ...
  const int num_kernels = batch_size * query_length * num_heads * channels;
  const int num_actual_kernels = batch_size * query_length * num_heads * channels;
  if (channels > 1024) {
    if ((channels & 1023) == 0) {
    ...
    switch(channels) {
      case 1:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 1>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 2:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 2>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 4:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 4>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 8:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 8>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 16:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 16>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 32:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v1<data_t, 32>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 64:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2<data_t, 64>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 128:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2<data_t, 128>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 256:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2<data_t, 256>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 512:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2<data_t, 512>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
        ...
        break;
      case 1024:
        deformable_attn_cuda_kernel_backward_shm_blocksize_aware_reduce_v2<data_t, 1024>
        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads,
            0, stream>>>(
                      num_kernels,
                      grad_out,
...
std::vector<paddle::Tensor> MSDeformableAttnCUDABackward(
    ...
    auto grad_sampling_locations = paddle::Tensor(paddle::PlaceType::kGPU);
    grad_sampling_locations.reshape(sampling_locations.shape());
    auto grad_attention_weights = paddle::Tensor(paddle::PlaceType::kGPU);
    grad_attention_weights.reshape(attention_weights.shape());
    ...
    deformable_attn_cuda_backward<float>(
        ...
        attention_weights.data<float>(),
        batch_size, value_length, num_heads, channels, num_levels,
        query_length, num_points,
        grad_value.mutable_data<float>(),
        grad_sampling_locations.mutable_data<float>(),
        grad_attention_weights.mutable_data<float>());
