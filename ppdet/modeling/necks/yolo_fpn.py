...
class PPYOLODetBlockCSPv2(nn.Layer):
    def __init__(self,
                 cfg,
                 ch_in,
                 ch_out,
        ...
        Args:
            ...
            data_format (str): data format, NCHW or NHWC
        """
        super(PPYOLODetBlockCSPv2, self).__init__()
        self.data_format = data_format
        self.conv1 = ConvBNLayer(
            ch_in,
            ch_out,
...
class PPYOLOPANv2(nn.Layer):
    ...
    def __init__(self,
        ...
        """
        super(PPYOLOPANv2, self).__init__()
        assert len(in_channels) > 0, "in_channels length should > 0"
        assert num_extra_levels >= 0, "num_extra_levels length should >= 0"

        self.in_channels = in_channels
        self.out_channel = out_channel
        self.num_extra_levels = num_extra_levels
        self.num_levels = len(in_channels) + self.num_extra_levels
        ...
        for in_c in self.in_channels:
            self.lateral_convs.append(
                ConvBNLayer(
                    in_c, self.out_channel, 1, act='', data_format=data_format))
        ...
        for i in range(self.num_levels):
            ...
            for j in range(self.conv_block_num):
                base_cfg += [
                    ...
                    [
                        ...
                        [self.out_channel, self.out_channel, 1], dict(
                            padding=0, act=act, norm_type=norm_type)
                    ],
                    [
                        '{}.1'.format(j), ConvBNLayer,
                        [self.out_channel, self.out_channel, 3], dict(
                            padding=1, act=act, norm_type=norm_type)
                    ]
                ]
            ...
            if i == 0 and self.spp:
                base_cfg[3] = [
                    'spp', SPP, [self.out_channel * 4, self.out_channel, 1],
                    dict(
                        pool_size=[5, 9, 13], act=act, norm_type=norm_type)
                ]
            ...
            fpn_block = self.add_sublayer(
                name,
                PPYOLODetBlockCSPv2(cfg, self.out_channel, self.out_channel,
                                    act, norm_type, name, data_format))
            self.fpn_blocks.append(fpn_block)

        # pan: P3 -> P7
        self.pan_blocks = []
        self.pan_down = nn.LayerList()
        for i in range(self.num_levels - 1):
            ...
            self.pan_down.append(
                ConvBNLayer(
                    ch_in=self.out_channel,
    ...
    def forward(self, blocks, for_mot=False):
        ...
        feats = []
        for conv_layer, feature in zip(self.lateral_convs, blocks):
            feats.append(conv_layer(feature))
        if self.num_extra_levels > 0:
            feat = blocks[-1]
        ...
        fpn_feats = []
        for i, (feature,
                fpn_block) in enumerate(zip(feats[::-1], self.fpn_blocks)):
            if i == 0:
                route, tip = fpn_block(feature)
            else:
                ...
                route, tip = fpn_block(feature + route)
            fpn_feats.append(tip)
        # pan
        pan_feats = []
        for i, feature in enumerate(fpn_feats[::-1]):
            if i == 0:
                pan_feats.append(feature)
                route = feature
            else:
                route = self.pan_down[i - 1](route)
                route, tip = self.pan_blocks[i - 1](route + feature)
                pan_feats.append(tip)
        return pan_feats
    ...
    def out_shape(self):
        return [
            ShapeSpec(channels=self.out_channel) for _ in range(self.num_levels)
        ]
