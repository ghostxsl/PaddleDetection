...
class BasicBlock(nn.Layer):
    def __init__(self, channels, act='swish', shortcut=True):
        super(BasicBlock, self).__init__()
        self.conv1 = ConvBNLayer(channels, channels, padding=1, act=act)
        self.conv2 = RepVggBlock(channels, channels, act=act)
...
class CSPStage(nn.Layer):
    def __init__(self, channels, block_fn='BasicBlock', n=3, act='swish'):
        ...
        for i in range(n):
            self.convs.add_sublayer(
                str(i), eval(block_fn)(ch_mid, act=act, shortcut=False))
...
class CSPPAN(nn.Layer):
    def __init__(self,
        ...
        self.conv_up = nn.LayerList([
            CSPStage(
                self.channels, block_fn, block_num, act=act)
    ...
    def forward(self, feats):
        ...
        for i, feature in enumerate(feats[::-1]):
            if i == 0:
                ...
                up_feature = F.interpolate(up_feature, scale_factor=2.)
    ...
    def __init__(self,
        ...
        for in_c in in_channels:
            self.lateral_convs.append(
                ConvBNLayer(
                    in_c, self.out_channel, 1, act=act))
        ...
        self.pan = CSPPAN(
            out_channel, self.num_levels, block_num, block_fn, act=act)
