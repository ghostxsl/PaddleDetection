...
def compute_bbox3d_corners_centers(center_2d,
                                   depth,
                                   size,
                                   rotation_yaw,
                                   K_inv,
...
def point3d_to_image(points, K):
    r"""
        ...
        Args:
            points_3d (Tensor): [..., 3, m]
            K (Tensor): [..., 4, 4]
        Return:
            points_2d (Tensor): [..., 2, m]
    """
    points_shape = points.shape
    points_shape[-2] = 1
    ones = paddle.ones(points_shape)
    points = paddle.concat([points, ones], axis=-2)
    points_2d = paddle.matmul(K, points)
    x, y, z, _ = paddle.chunk(points_2d, 4, axis=-2)
    points_2d = paddle.concat([x / z, y / z], axis=-2)
    return points_2d
