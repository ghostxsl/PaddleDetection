...
class Resize3d(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'bbox' in sample and len(sample['bbox']) > 0:
            sample['bbox'] = self.apply_bbox(
                sample['bbox'], [im_scale_x, im_scale_y], [resize_w, resize_h])

        if 'bbox_2d' in sample and len(sample['bbox_2d']) > 0:
            sample['bbox_2d'] = self.apply_bbox(sample['bbox_2d'],
                                                [im_scale_x, im_scale_y],
                                                [resize_w, resize_h])

        if 'center_2d' in sample and len(sample['center_2d']) > 0:
            sample['center_2d'] = self.apply_point(sample['center_2d'],
                                                   [im_scale_x, im_scale_y],
                                                   [resize_w, resize_h])
...
class RandomFlip3d(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if 'center_2d' in sample and len(sample['center_2d']) > 0:
                sample['center_2d'] = self.apply_keypoint(sample['center_2d'],
                                                          width)
