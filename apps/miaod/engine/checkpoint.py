...
def load_weight(model, weight, optimizer_list=None, ema=None):
    if is_url(weight):
    ...
    # load optimizer
    total_epoch_id = 0
    if optimizer_list is not None and os.path.exists(path + '.pdopt'):
        opt_state_dict = paddle.load(path + '.pdopt')
        if 'total_epoch_id' in opt_state_dict:
            total_epoch_id = opt_state_dict.pop('total_epoch_id')
        # to solve resume bug, will it be fixed in paddle 2.0
        for i, optimizer in enumerate(optimizer_list):
            # for key in optimizer.state_dict().keys():
            #     if not key in state_dict_.keys():
            #         state_dict_[key] = optimizer.state_dict()[key]
            optimizer.set_state_dict(opt_state_dict[i])

        if ema_state_dict is not None:
            ema.resume(ema_state_dict,
                       opt_state_dict[0]['LR_Scheduler']['last_epoch'])
    elif ema_state_dict is not None:
        ema.resume(ema_state_dict)
    return total_epoch_id
...
def save_model(model,
    ...
    Args:
        ...
        optimizer_list (list[paddle.optimizer.Optimizer]): the Optimizer instance to
            save optimizer states, multiple optimizer can be saved.
        save_dir (str): the directory to be saved.
        save_name (str): the path to be saved.
        total_epoch_id (int): the total_epoch_id.
    ...
    # save optimizer
    state_dict = {'total_epoch_id': total_epoch_id}
    state_dict.update({k: v.state_dict() for k, v in enumerate(optimizer_list)})
