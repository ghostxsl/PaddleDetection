...
import numba
import io as sysio
from tools_3d.kitti_object_eval_python.rotate_iou import rotate_iou_gpu_eval
...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if (gt_name == current_cls_name):
            valid_class = 1
        elif (current_cls_name == "Pedestrian".lower() and
              "Person_sitting".lower() == gt_name):
            valid_class = 0
...
def compute_statistics_jit(overlaps,
    ...
    for i in range(gt_size):
        ...
        for j in range(det_size):
            ...
            if (not compute_fp and (overlap > min_overlap) and
                    dt_score > valid_detection):
                det_idx = j
                valid_detection = dt_score
            elif (compute_fp and (overlap > min_overlap) and
                  (overlap > max_overlap or assigned_ignored_det) and
                  ignored_det[j] == 0):
                max_overlap = overlap
                det_idx = j
                valid_detection = 1
                assigned_ignored_det = False
                  ...
                  (valid_detection == NO_DETECTION) and ignored_det[j] == 1):
                det_idx = j
                valid_detection = 1
...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    for j, num_part in enumerate(split_parts):
        ...
        for i in range(num_part):
            ...
            overlaps.append(parted_overlaps[
                j][gt_num_idx:gt_num_idx + gt_box_num, dt_num_idx:dt_num_idx +
                   dt_box_num])
